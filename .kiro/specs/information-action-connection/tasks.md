...
- [ ] 1. 一致度計算関数の実装
  - [ ] 1.1 `src/core/similarity.ts`を作成し、`calculateSimilarity`関数を実装
...
- [ ] 2. 知識ボーナス計算の実装
  - [ ] 2.1 `src/core/similarity.ts`に`calculateKnowledgeBonus`関数を追加
    - similarity ≤ 0.5 → 1.0
...
- [ ] 3. Universe設定の拡張
  - [ ] 3.1 `UniverseConfig`に`knowledgeBonusEnabled`フィールドを追加
    - デフォルト値: true
    - _Requirements: 4.4_

- [ ] 4. 修復処理への統合
  - [ ] 4.1 `executeRepairArtifact`メソッドを修正
    - 一致度計算を追加
    - ボーナスを修復量に適用
...
- [ ] 5. 観測システムの拡張
  - [ ] 5.1 `SimulationEvent`型に`similarity`と`knowledgeBonus`フィールドを追加
    - `artifactRepaired`イベントを拡張
    - _Requirements: 2.5, 3.1_
  - [ ] 5.2 `SimulationStats`に知識関連メトリクスを追加
    - `avgSimilarity`, `repairCount`, `bonusAppliedCount`
    - _Requirements: 3.2, 3.3, 3.4_

- [ ] 6. CLIオプションの追加
  - [ ] 6.1 `--knowledge-bonus`オプションを追加
    - `on`/`off`で切り替え可能
    - _Requirements: 4.4_
